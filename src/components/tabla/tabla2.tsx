...
export interface ITabla2Props extends IPropsTabla {
	height?: string
	keyTable?: string
	labelTable?: string
...
export interface ITabla2State {
 ...
 const Tabla2 = (props: ITabla2Props) => {
	...
	const preparseData = () => {
		...
		if (props.showAll) {
			...
			for (let i = 0; i < parseData.length; i++) {
				...
				for (const [key, _col] of THIS.columnsMap.entries()) {
					const _val = reg[key]
					...
					if (THIS.dataMap.has(key)) {
						const _dataList = THIS.dataMap.get(key) || []
						_dataList.push(_val)
	...
	const TablaTH = (propsTH: ITablaTHprops) => {
		let styleFixd: CSSProperties = propsTH.showAll
			? {
					position: 'sticky',
					zIndex: '100',
					top: 0,
			...
			>
				<div className="flex justify-between  items-center w-full ">
					<div className="text-white dark:text-white w-full">
						...
						{propsTH.column?.setColumnHeader &&
							propsTH.column.setColumnHeader()}
						{/* {!(props.exportExcel || props.btnAddRegistro || props.importExcel) && <label>{propsTH.column.label}</label>} */}
	...
	const contentTable = () => {
		return (
			<Fragment>
				...
				<div
					className="relative"
					style={{ overflowX: 'auto', height: props?.height }}
				>
	...
	return (
		<Fragment>
			...
			>
				{!props?.sinBordes && <Card02 styles={{ height: props.heightCard || ''}}>{contentTable()}</Card02>}
				{props?.sinBordes && <div className="m-2">{contentTable()}</div>}
			</div>
			{showModalExcel && (
				<ModalExcelImport
					getRegistros={(regs) => {
						if (props.getRegsExcel) {
							props.getRegsExcel(regs)
						}
					}}
					onClose={(closed) => {
						setShowModalExcel(!closed)
					}}
			...
			reader.onload = () => {
				workbook.xlsx
					...
					.catch((error) => {
						console.log(error)
						// Loading.remove()
						// Notify.failure('El archivo Excel no pudo ser leído')
					})
			}
		} else {
					...
					<>
						<div className="justify-between flex w-full">
                            ...
                            if (props.onClose) props.onClose(true)
                               ...
                               className="block rounded-full px-2 py-1 cursor-pointer bg-gray-100 hover:bg-gray-200 active:bg-gray-200"><span
								...
								{_row.map((_cell, index) => {
									...
									return (
										...
										>
											...
											{col?.render && col.render(_cell, _data[k], k)}
										</td>
									)
								})}
							</tr>

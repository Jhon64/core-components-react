...
export class ManageColumns extends Component<IManageColumnsProps, IManageColumnsState>{
    ...
    componentDidUpdate(prevProps: Readonly<IManageColumnsProps>, prevState: Readonly<any>, snapshot?: any): void {
    ...
    render(): React.ReactNode {
        ...
        return <React.Fragment>
            <div className='relative manage-columns-component'>
                <Button2 secondary outline size='md' icon='fa fa-cog' onClick={() => this.handleClickActive()}
                    class={state.isActive ? "active" : ''} />
                <div className={("absolute card  bg-white shadow-sm shadow-gray-500  right-0 rounded ") + (state.isActive ? "active" : '')} style={{ minWidth: '14rem', zIndex: 99 }}>
                    ...
                    <div className="py-3 px-5">
                        {state.stateColumns.map((x: any) => {
                            return <div className="flex">
                                <CheckBox2 checked={state.idsHiddenSet?.has(x._key)}
                                    onChange={(v) => {
                                ...
                                <label className="text-gray-600 font-light" style={{ fontSize: '12px' }}>{x.label}</label>
                            </div>
                        })}
                        <Button2 outline info text="Aplicar" class="w-full mt-2" onClick={() => {
